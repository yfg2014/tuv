...
class Review {
	...
	public function add($params){
		...
		DBUtil::insert_tb($db, $this->dbtable['hr_review'], $params);
		$id = $db->insert_id();
		return $id;
	}
	...
	public function update($id, $params){
		...
		DBUtil::update_tb($db, $this->dbtable['hr_review'], $params, "id='{$id}'");
	...
	public function delete($id){
		...
		DBUtil::Del($db, $this->dbtable['hr_review'], "id='{$id}'");
		return true;
	}
	...
	public function query($id,$params = array()){
		...
		$result = $db->get_one("SELECT {$field} FROM {$this->dbtable['hr_review']} WHERE id='{$id}'");
	...
	public function toArray($where,$params){
		...
		$params == array() ? $field = '*' : $field = implode(',', $params);
		$q = $db->query("SELECT {$field} FROM {$this->dbtable['hr_review']} WHERE {$where}");
	...
	public function save($id,$params){
		if((int)$id > 0){
			...
			LogRW::logWriter('','体系管理人员修改');
		}elseif ((int)$id == 0) {
			$id = $this->add($params);
			LogRW::logWriter('','体系管理人员登记');
		}
