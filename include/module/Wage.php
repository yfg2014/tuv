...
class Wage {
	...
	public function add($params){
		...
		DBUtil::insert_tb($db, $this->dbtable['hr_wage'], $params);
		$id = $db->insert_id();
		return $id;
	}
	...
	public function update($id, $params){
		...
		DBUtil::update_tb($db, $this->dbtable['hr_wage'], $params, "id='{$id}'");
	...
	public function delete($id){
		...
		DBUtil::Del($db, $this->dbtable['hr_wage'], "id='{$id}'");
		return true;
	}
	...
	public function query($id,$params = array()){
		...
		$result = $db->get_one("SELECT {$field} FROM {$this->dbtable['hr_wage']} WHERE id='{$id}'");
	...
	public function toArray($where,$params){
		...
		$params == array() ? $field = '*' : $field = implode(',', $params);
		$q = $db->query("SELECT {$field} FROM {$this->dbtable['hr_wage']} WHERE {$where}");
	...
	public function save($id,$params){		
		if((int)$id > 0){
			...
			LogRW::logWriter('','人员工资修改');
		}elseif ((int)$id == 0) {
			$id = $this->add($params);
			LogRW::logWriter('','人员工资登记');
		}
