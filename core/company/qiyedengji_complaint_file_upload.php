...
$save_path = UPLOAD_DIR.$setup_uploadpath[3].'/';
...
if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$filename)) {
	...
	$value = array(
		...
		'path' => $setup_uploadpath[3],
		'fname' => $filename,
		'filetype' => $file_extension,
	);
	if($FileID > 0){
		$rows = $db->get_one("SELECT * FROM {$dbtable['mk_company_complaint']} WHERE id='".$FileID."'");
		DBUtil::Update_tb($db,$dbtable['mk_company_complaint'],$value,"id='{$FileID}'");
		$id = $FileID;
		LogRW::logWriter('',$_FILES[$upload_name]['name'].' 投诉附件修改上传');
		$file = UPLOAD_DIR.$rows['path'].'/'.$rows['fname'];
		$file = str_replace('\\','/',$file);
		if (file_exists($file))@unlink($file);
	}else{
		DBUtil::insert_tb($db, $dbtable['mk_company_complaint'], $value);
		$id = mysql_insert_id();
		LogRW::logWriter('',$_FILES[$upload_name]['name'].' 投诉附件上传');
	}
	echo $id;
}
